...
def active_neighbors(
    state: Dict[Tuple[int, int, int, int], str], point: Tuple[int, int, int, int]
) -> int:
    (point_x_index, point_y_index, point_z_index, point_w_index) = point
    ...
    for x_index in range(point_x_index - 1, point_x_index + 2):
        for y_index in range(point_y_index - 1, point_y_index + 2):
            for z_index in range(point_z_index - 1, point_z_index + 2):
                for w_index in range(point_w_index - 1, point_w_index + 2):
                    key = (x_index, y_index, z_index, w_index)
                    if point == key:
                        continue
...
def get_neighbors(point: Tuple[int, int, int, int]) -> List[Tuple[int, int, int, int]]:
    (point_x_index, point_y_index, point_z_index, point_w_index) = point
    ...
    for x_index in range(point_x_index - 1, point_x_index + 2):
        for y_index in range(point_y_index - 1, point_y_index + 2):
            for z_index in range(point_z_index - 1, point_z_index + 2):
                for w_index in range(point_w_index - 1, point_w_index + 2):
                    neighbor = (x_index, y_index, z_index, w_index)
                    if point == neighbor:
                        continue
...
def transition_state(
    initial_state: Dict[Tuple[int, int, int, int], str]
...
def solve() -> str:
    state: Dict[Tuple[int, int, int, int], str] = {}
    ...
    for line in file:
        ...
        for x_index, character in enumerate(characters):
            state[(x_index, y_index, 0, 0)] = character
        y_index += 1
